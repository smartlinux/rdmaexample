...
/*
 * cm_atomic_lock_client.c
 *
 * Connects to cm_atomic_lock_server, acquires a remote lock via RDMA CAS,
 * writes a message under the lock, then releases the lock.
 *
 * Build:
 *   gcc -O2 -Wall -std=c11 cm_atomic_lock_client.c -o cm_atomic_lock_client -lrdmacm -libverbs
 *
 * Run:
 *   ./cm_atomic_lock_client <server_ip> <port> [message] [hold_ms] [local_ip]
 */
...
static int try_lock_cas(struct ibv_qp *qp, struct ibv_cq *cq,
                        uint64_t lock_addr, uint32_t rkey,
                        uint64_t *result, struct ibv_mr *result_mr) {
    struct ibv_sge sge;
    memset(&sge, 0, sizeof(sge));
    ...
    if (ibv_post_send(qp, &wr, &bad)) die("ibv_post_send(CAS)");

    poll_one(cq, IBV_WC_COMP_SWAP);

    return (*result == 0);
}

static void rdma_write(struct ibv_qp *qp, struct ibv_cq *cq,
...
int main(int argc, char **argv) {
    ...
    uint64_t lock_addr = base + 0;
    uint64_t data_addr = base + data_off;

    printf("[client] connected: base=0x%016" PRIx64 " rkey=0x%x lock=0x%016" PRIx64 " data=0x%016" PRIx64 "\n",
           base, rkey, lock_addr, data_addr);

    uint64_t *cas_result = NULL;
    if (posix_memalign((void **)&cas_result, 8, sizeof(uint64_t))) die("posix_memalign(cas_result)");
    *cas_result = 0;
    struct ibv_mr *cas_mr = ibv_reg_mr(pd, cas_result, sizeof(uint64_t), IBV_ACCESS_LOCAL_WRITE);
    if (!cas_mr) die("ibv_reg_mr(cas_result)");

    uint64_t *zero = NULL;
    if (posix_memalign((void **)&zero, 8, sizeof(uint64_t))) die("posix_memalign(zero)");
    *zero = 0;
    struct ibv_mr *zero_mr = ibv_reg_mr(pd, zero, sizeof(uint64_t), IBV_ACCESS_LOCAL_WRITE);
    if (!zero_mr) die("ibv_reg_mr(zero)");
    ...
    int spins = 0;
    for (;;) {
        int ok = try_lock_cas(id->qp, cq, lock_addr, rkey, cas_result, cas_mr);
        if (ok) break;

        spins++;
        if ((spins & 0x3f) == 0) usleep(100);
    }
    printf("[client] lock acquired after %d failed tries\n", spins);

    if (hold_ms > 0) usleep((useconds_t)hold_ms * 1000);

    rdma_write(id->qp, cq, msg, 256, msg_mr, data_addr, rkey);
    printf("[client] wrote data: '%s'\n", msg);

    rdma_write(id->qp, cq, zero, (uint32_t)sizeof(uint64_t), zero_mr, lock_addr, rkey);
    printf("[client] lock released\n");
