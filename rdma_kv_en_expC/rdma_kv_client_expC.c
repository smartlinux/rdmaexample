...
/*
 * Basic operations:
 *   ./rdma_kv_client_expC <server_ip> <port> put <key> <value> [--rnr-retry N]
 *   ./rdma_kv_client_expC <server_ip> <port> get <key> [--rnr-retry N]
 *
 * Experiment C (RNR / receive resource) helper:
 *   ./rdma_kv_client_expC <server_ip> <port> bench_get <key> <iters> [--rnr-retry N] [--progress M]
 *
 * Notes:
...
static int poll_one_cq(struct ibv_cq *cq, struct ibv_wc *out_wc, int timeout_ms) {
    /*
     * Poll a single WC from the CQ.
     *
     * Return:
     *   0  : got a WC (success or error; check out_wc->status)
     *  -1  : timed out (only if timeout_ms > 0)
     */
    int waited_ms = 0;
    ...
    while (1) {
        ...
        if (out_wc->status != IBV_WC_SUCCESS) {
            fprintf(stderr,
                    "WC error: %s (status=%d) opcode=%d wr_id=%" PRIu64 "",
                    ibv_wc_status_str(out_wc->status),
                    out_wc->status, out_wc->opcode, out_wc->wr_id);
        }
        return 0;
    }
}
...
struct client {
    ...
    // Experiment C knobs (sender-side):
    int retry_count;        // RC retry count (NACK/timeout); 7 = max
    int rnr_retry_count;    // RNR retry count; 7 = max (often effectively 'infinite' on many stacks)
};
...
static void do_put(struct client *c, const char *key, const char *value) {
    ...
    while (!got_send || !got_recv) {
        if (poll_one_cq(c->cq, &wc, 0) < 0) {
            fprintf(stderr, "CQ poll timed out while waiting for request completion\n");
            exit(1);
        }
        if (wc.status != IBV_WC_SUCCESS) {
            if (wc.status == IBV_WC_RNR_RETRY_EXC_ERR) {
                fprintf(stderr, "Hint: RNR retry exceeded. The peer likely had no posted RECV WQE.\n");
            }
            exit(1);
        }
    ...
    if (poll_one_cq(c->cq, &wc, 0) < 0) {
        fprintf(stderr, "Timeout waiting for RDMA_WRITE completion\n");
        exit(1);
    }
    if (wc.status != IBV_WC_SUCCESS) {
        exit(1);
    ...
    if (poll_one_cq(c->cq, &wc, 0) < 0) {
        fprintf(stderr, "Timeout waiting for WRITE_WITH_IMM completion\n");
        exit(1);
    }
    if (wc.status != IBV_WC_SUCCESS) {
        exit(1);
...
static void do_get(struct client *c, const char *key) {
    ...
    if (poll_one_cq(c->cq, &wc, 0) < 0) {
        fprintf(stderr, "Timeout waiting for CQ completion\n");
        exit(1);
    }
    if (wc.status != IBV_WC_SUCCESS) {
        exit(1);
...
static uint32_t do_get_meta(struct client *c, const char *key) {
    /*
     * A "metadata-only" GET:
     *   - send KV_GET_REQ
     *   - receive KV_GET_RESP
     *   - do NOT RDMA_READ the value payload
     *
     * This is useful for Experiment C to stress the control plane without moving large data.
     */
    size_t key_len = strlen(key);
    if (key_len == 0 || key_len > KEY_MAX) {
        fprintf(stderr, "Key length must be 1..%u bytes\n", KEY_MAX);
    ...
    while (!got_send || !got_recv) {
        if (poll_one_cq(c->cq, &wc, 0) < 0) {
            fprintf(stderr, "CQ poll timed out while waiting for GET meta completion\n");
            exit(1);
        }
        if (wc.status != IBV_WC_SUCCESS) {
            if (wc.status == IBV_WC_RNR_RETRY_EXC_ERR) {
    ...
    return ntohl(c->recv_msg->status);
}

static void do_bench_get(struct client *c, const char *key, int iters, int progress) {
    printf("[client] bench_get: iters=%d progress=%d rnr_retry=%d retry=%d\n",
           iters, progress, c->rnr_retry_count, c->retry_count);

    for (int i = 1; i <= iters; i++) {
        uint32_t st = do_get_meta(c, key);
        (void)st;

        if (progress > 0 && (i % progress) == 0) {
            printf("[client] bench_get progress: %d/%d\n", i, iters);
        }
    }
    printf("[client] bench_get done.\n");
}
...
static void connect_cm(struct client *c, const char *server_ip, const char *port_str) {
    ...
    param.retry_count = (uint8_t)c->retry_count;
    param.rnr_retry_count = (uint8_t)c->rnr_retry_count;
...
int main(int argc, char **argv) {
    if (argc < 5) {
        fprintf(stderr,
                "Usage:"
                "  %s <server_ip> <port> put <key> <value> [--rnr-retry N]"
                "  %s <server_ip> <port> get <key> [--rnr-retry N]"
                "  %s <server_ip> <port> bench_get <key> <iters> [--rnr-retry N] [--progress M]",
    ...
    if (strcmp(op, "put") == 0) {
        ...
        key = argv[4];
        opt_i = 5;
    } else if (strcmp(op, "bench_get") == 0) {
        if (argc < 6) {
            fprintf(stderr, "bench_get requires <key> <iters>");
            return 2;
        }
        key = argv[4];
        iters = atoi(argv[5]);
        ...
        fprintf(stderr, "Unknown op: %s", op);
        return 2;
    }

    /* Parse optional flags (must appear after op arguments). */
    for (int i = opt_i; i < argc; i++) {
        if (strcmp(argv[i], "--rnr-retry") == 0 && i + 1 < argc) {
            rnr_retry_count = atoi(argv[++i]);
    ...
    if (retry_count > 7) retry_count = 7;
    if (rnr_retry_count < 0) rnr_retry_count = 0;
    if (rnr_retry_count > 7) rnr_retry_count = 7;

    struct client c;
    memset(&c, 0, sizeof(c));
    c.retry_count = retry_count;
    c.rnr_retry_count = rnr_retry_count;
    ...
    if (strcmp(op, "put") == 0) {
        ...
        do_bench_get(&c, key, iters, progress);
    }
